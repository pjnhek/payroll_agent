# v3 — Production-Ready Codebase (ARCHIVED)

**Status:** Complete
**Phases:** 4 (12–15)
**Requirements:** 16/16 satisfied
**Timeline:** 2026-07-08 → 2026-07-13 (6 days)
**Git range:** `949d933..cdcae40` — 227 commits, 417 files changed (+36,004 / −61,010)
**Audit:** [v3-MILESTONE-AUDIT.md](../v3-MILESTONE-AUDIT.md) — PASSED (16/16 reqs, 4/4 phases, 5/5 integration seams)
**Tag:** `v3`

**Goal:** Make the codebase read as production-quality — enforced CI, right-sized modules, full
type-checking, constraint-documenting comments. No pipeline or money behavior change.
...
- **Enforced CI** — `ci.yml` runs `ruff check`, the full hermetic pytest suite, and `mypy --strict`
  as three blocking jobs. The real-Postgres concurrency proofs now gate pull requests, not just
  post-merge master — they are the only tests in the repo that touch a real database.
- **Right-sized modules** — `main.py` 1,857 → 16 lines (5 APIRouters + pipeline_glue + templating);
  `repo.py` 1,765 → per-aggregate `app/db/repo/` package with a stable facade; `orchestrator.py`
  1,843 → 1,029 (alias_learning / clarification / delivery carved out). Cross-module `_private`
  imports promoted to public names, guarded by an AST test.
- **Full type-checking** — `mypy --strict` clean over **117 files** (app + eval + scripts + tests),
  blocking in CI.
- **Constraint-documenting comments** — ticket-ID/provenance comments stripped repo-wide, replaced
  with comments that state the constraint and the failure it prevents. Enforced by a CI guard pinned
  against the real ticket-family inventory.

### Three real defects found by phases scoped as "hygiene"

1. **The eval chart was lying.** A mislabeled fixture dumped an intentional phantom-employee miss
   into the `exact` bucket, so the chart reported exact-match extraction failing at 0.96 when it had
   never failed. Filed as a cosmetic todo; it was a real defect in the one artifact recruiters read.
2. **A path traversal** that actually rendered a file from outside `eval/fixtures/` onto the eval page.
3. **A prompt-echo leak** — the LLM retry prompt fed the model's own output back to the provider.

### Key lessons

- **A guard is only as good as its scan scope and vocabulary, and neither is visible from a passing
  run.** The comment-provenance guard was green *because it was blind*, four separate times — the
  last exposing 8 live rot references, two in production source. Fixed structurally: the pattern
  table is now asserted against the real family inventory harvested from git history.
- **Always run a review round that reviews the FIX.** Cross-AI round 1 found the prompt-echo leak;
  round 2 found that the *fix* was incomplete in exactly the way it claimed to have closed
  (`ValidationError.input` scrubbed, `loc` left open — and under `extra="forbid"`, a model-invented
  field name lands in `loc`). Round 3 confirmed the correction.
- **Prove a guard can fail before trusting it.** Mutation-testing the epoch arbiter showed that
  narrowing it to three columns fails one test while another still passes — concrete evidence the
  weaker test alone would have missed a clobbered delivered email.

### Tech debt carried forward

| Item | Severity |
|------|----------|
| Phase 12 has no VALIDATION.md; phase 15's is an unfilled draft template | low (process artifact) |
| 3 deferred polish todos: frontend progressive enhancement, paystub YTD columns, eval chart restyle | low |
| Stale quick-task pointer `260621-11x` (work done, tracking file never closed) | low |

### Money-path safety

No pipeline or money behavior changed. Verified by AST-diffing the money core against the
pre-milestone base with docstrings stripped and string constants blanked: **all 194 numeric/Decimal
literals in `tax_tables_2026.py` are identical**, as are those in `federal_withholding.py` and
`calculate.py`. The deterministic-decisioning thesis survived three refactoring phases —
`decide.py` still contains no scoring concept (`confidence` count: 0).
