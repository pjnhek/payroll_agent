...
- **D-A1-04 [informational]:** Outbound-send idempotency — **this is CLAR-04, which belongs to the dashboard phase (Phase 5), not Phase 2.** (Tagged `[informational]`: this is a deferral/non-deliverable for Phase 2, so the decision-coverage gate does not require a Phase 2 plan to own it. Phase 2 only records sent Message-IDs as forward-compatible groundwork; the hard idempotency guarantee is a Phase 5 plan decision.) Phase 2's `EMAIL-01` stub does store every outbound message with a synthetic Message-ID, and Phase 2 should record per-run sent-state so the design is forward-compatible — but the hard idempotency guarantee (no duplicate clarification/confirmation on retry/re-trigger) is a Phase 5 deliverable, not a Phase 2 acceptance criterion. Do not over-build this in Phase 2 beyond recording sent Message-IDs.
...
- **D-A2-01:** Test strategy — **mock/recorded responses by default, live opt-in**. Unit tests run the stages against stubbed/recorded fixture responses (deterministic, free, no network, CI-safe). A separate **env-gated opt-in mode** (mirroring the live-DB tests' two-factor guard) hits the real DeepSeek/Kimi APIs for periodic sanity checks. Real-model *accuracy* is measured in the eval (Phase 4), not the test suite.
  - **Live-vs-mock run marker (post-AI-SPEC gap-check, NICE-TO-HAVE):** record a per-run marker (a decision-detail key, log field, or column — planner's discretion) so a given decision is provably from a **live** model call vs a replayed fixture. This is the on-camera honesty crux of D-A4-01a ("this was a real model call, not a recording") — the marker lets logs/output prove it. The opt-in env flag NAME is Claude's discretion (mirror the live-DB two-factor pattern); only the marker's existence is asked for here. NICE-TO-HAVE, not a Phase 2 exit gate.
...
- **D-A3-03:** Decision object (LLM-08) — persisted as the existing Pydantic **`Decision` contract** (from Phase 1 `app/models/`) stored in a **JSONB column on `payroll_runs`**. Fields: `model_action`, `gate_triggered`, `gate_reasons`, `final_action`, `unresolved_names`, `missing_fields`, `confidence`, `reasons`. One typed object the orchestrator/dashboard/eval all read; they branch only on `final_action`. The `payroll_runs.decision` JSONB column **already exists** in `schema.sql` from Phase 1 — the planner confirms it (and any new column added in D-A3-05) keeps the status-drift guard green; no re-add needed.
  - **Confidence-collapse rule (post-AI-SPEC gap-check, D-A3-03a):** reconciliation emits a confidence **per name** (`NameMatchResult.confidence`), but `Decision.confidence` is a **single scalar**. The collapse is **`min()`** — the weakest-link name drives the audit scalar; a clean run with no LLM-layer names collapses to `1.0`. **Critical:** the 0.8 gate itself evaluates **each name's own confidence** against the threshold inside `decide.py` — it does NOT gate on the collapsed scalar. The scalar `Decision.confidence` is for audit/eval/dashboard display only; gating on an average would let one 0.6 name hide behind three 1.0s and sail past 0.8, which is exactly the failure the code gate exists to prevent.
...
- **D-A3-05 (post-AI-SPEC gap-check):** Per-name reconciliation detail MUST be persisted — the full `list[NameMatchResult]` for the run (each name's `submitted_name`, `matched_employee_id`, `match_type`, per-name `confidence`, `reason`) is the source of truth for **Phase 4's name-reconciliation-accuracy metric** and **Phase 5's per-name dashboard column** (the "the model was willing; the code said no" money shot). It must NOT be discarded after `decide.py` computes the gate. Persistence home is the planner's discretion between **(a)** a new `payroll_runs.reconciliation JSONB` column or **(b)** nesting it inside the existing `decision` JSONB — either keeps the status-drift guard green. The eval rides the same production functions (PITFALLS P8/P10), so Phase 2 persisting this now is what lets Phase 4 score name-recon **without re-running the model** (reproducibility); retrofitting it later is expensive.
