...
| INGEST-02 | early | T-PII | inbound stored with headers; quoted history/signatures stripped before extraction | unit | `pytest tests/test_ingest.py::test_body_cleaned -x` | ❌ W0 | ⬜ pending |
...
| LLM-08 | mid | — | `Decision` persisted to `payroll_runs.decision`; round-trips; per-name `reconciliation` persisted (D-A3-05) | integration | `pytest tests/test_persistence.py::test_decision_roundtrip -x` | ❌ W0 | ⬜ pending |
| LLM-09 | mid | — | one-to-one mapping collisions (dup name / two→one emp / name→no emp) gate to clarify | unit | `pytest tests/test_gate.py::test_one_to_one_collisions -x` | ❌ W0 | ⬜ pending |
...
| DEMO-01 | late | — | both committed fixtures replay end-to-end via POST | integration (mocked LLM) | `pytest tests/test_demo_fixtures.py -x` | ❌ W0 | ⬜ pending |
