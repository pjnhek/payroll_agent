...
| Render free-tier request-duration ceiling ≥ the pump's worst-case drain time (~330s) | PUMP-01 / D-05 | Render's server-side request ceiling is undocumented (Open Question #2 / Assumption A1) | Live smoke against the deployed `/internal/pump` with a small backlog; confirm no proxy-level timeout truncates the response. If it does, tighten the D-05 wall-clock cap below the observed ceiling. |
