...
| OPS-02 / D-14 / D-13c | Outbound send calls `repo.get_outbound_references_chain(run_id)` to LOAD the prior accumulated References chain from DB rows, appends the new `in_reply_to` token to build the full chain, THEN writes `send_state='reserved'` row BEFORE `resend.Emails.send` call | Threading corruption on dropped/dup delivery (Integrity); send-without-record (Repudiation) | Durable threading from DB state — FakeConnection pre-populated with prior outbound row; test asserts accumulated chain contains BOTH prior tokens AND new message_id | unit (mock `resend.Emails.send` + FakeConnection) — xfail(strict=True) in 06-01, xfail removed in 06-04 | `uv run pytest tests/test_gateway.py -k "threading or references or reserved" -x -q` | ❌ W0 | ⬜ pending |
