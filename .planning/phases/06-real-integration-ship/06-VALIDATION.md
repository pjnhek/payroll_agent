...
| OPS-02 / HIGH-1 / D-01b / A5 | send_outbound passes `headers={"Message-ID": synthetic_id}` to resend.Emails.send; email_messages.message_id stores the SAME synthetic id; Resend's returned provider id stored separately; reply routing matches In-Reply-To against the synthetic id — no id mismatch | Reply routing fails after real send (Integrity/Spoofing) | HIGH-1 design: app owns the RFC Message-ID; synthetic id is both the outbound header and the DB routing key; A5 verified at the human round-trip gate (06-05) | unit (mock resend.Emails.send; assert headers["Message-ID"] == synthetic_id; assert DB message_id != provider_id) — xfail(strict=True) in 06-01, xfail removed in 06-04; A5 round-trip confirmation is manual (06-05) | `uv run pytest tests/test_gateway.py -k "message_id or reserved" -x -q` (Message-ID header assertion) | ❌ W0/W3 | ⬜ pending |
...
| OPS-02 / HIGH-4 / D-17 | Unsigned canonical InboundEmail-shaped POST (no data.email_id, no svix-* headers) → route returns 400 when ALLOW_UNSIGNED_FIXTURES=False (prod default); same POST → 200 when ALLOW_UNSIGNED_FIXTURES=True (dev mode) | Unsigned canonical bypass — prod forgery hole (Spoofing) | HIGH-4: allow_unsigned_fixtures=False rejects ALL unsigned POSTs regardless of shape; tests test_allow_unsigned_fixtures_canonical_shape_prod_default_returns_400 and test_allow_unsigned_fixtures_canonical_shape_dev_mode_returns_200 lock both paths | unit (no xfail — GREEN from 06-04 Task 2) | `uv run pytest tests/test_gateway.py -k "canonical_shape" -x -q` | ❌ W3 | ⬜ pending |
...
| Real email round-trip (HIGH-2 identity + HIGH-1 A5 + LOW-8): (a) production demo identity set — one business contact_email updated to real sender before round-trip; (b) send → reply → **three threading headers intact** on deployed service; (c) reply In-Reply-To matches synthetic message_id stored in DB (A5/HIGH-1 confirmed); LOG_WEBHOOK_DEBUG_IDS captures real header casing (A1) | OPS-02 / D-09b / HIGH-1 / HIGH-2 / LOW-8 | Real sender-client header survival; find_business_by_sender exact-match requires real email in DB (HIGH-2); A5 is unconfirmable without real send+reply; LOW-8 threading header evidence can only be collected here | Human: (0) update businesses.contact_email to real sender email (HIGH-2 setup); (1) enable LOG_WEBHOOK_DEBUG_IDS=true in Render env; (2) do personal send→reply→approve round-trip; (3) check WEBHOOK_DEBUG log for header_keys=[...] (A1); (4) compare reply In-Reply-To to DB message_id (A5/HIGH-1); (5) confirm email_id vs rfc_message_id (A6); (6) confirm all three threading headers present (LOW-8) |
