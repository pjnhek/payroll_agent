...
**Defined:** 2026-06-20
**Core Value:** A messy real-world payroll email goes in; a correct, human-approved payroll comes out — and every money-moving judgment call (name match, process-vs-clarify) is gated by code — deterministic, auditable, and never guesses. (Phase 2.1 superseded the original confidence-gate framing: the decision is pure code over resolution facts, with no score read anywhere.)
...
- [x] **LLM-04**: Deterministic name matching is the WHOLE matcher (Phase 2.1) — pure code resolves each submitted name against the roster as **exact** (casefold + whitespace-normalized full_name, unique) / **stored-alias** (a unique `known_aliases` hit) / **none** (anything else: no match, typo, first-time nickname, garbled, or ambiguous). There is no model classification layer; the resolver never guesses
- [x] **LLM-05**: The name resolver is deterministic (LLM-04) — there is no LLM reconciliation step and no confidence score. The only LLM judgment here is the optional **clarification-suggestion call** (Phase 2.1, D-21-05): for a name the resolver left unresolved, ONE cheap-tier call suggests the most likely intended roster employee, used ONLY to make the clarification email specific ("did you mean David Reyes?"). It is advisory COPY — it never feeds `decide` / `final_action`, and a suggested name must be an actual roster employee or it is dropped
- [x] **LLM-06**: Deterministic field validation produces a per-field issues list (presence, sanity bounds, numeric)
- [x] **LLM-07**: Decisioning is deterministic (Phase 2.1, D-21-01): `decide.py` computes a code-owned `final_action` purely from the resolution facts — it forces `request_clarification` on any **unresolved name**, any **run-level collision** (a name shared by 2+ employees, or two names mapping to one employee), or any **missing required field**, with `gate_reasons` listing exactly what triggered it. There is no model action and no confidence number — `decide.py` makes no LLM call, reads no score, and never branches on a model action. `final_action` is the SOLE branch source consumed downstream (orchestrator, dashboard, eval)
- [x] **LLM-08**: The deterministic decision object (`final_action`, `gate_reasons`, `unresolved_names`, `missing_fields`, plus the per-name `resolutions`) is persisted on the run for audit and the eval (Phase 2.1). The dead `model_action` / `gate_triggered` / `confidence` / `reasons` fields were removed with the confidence gate; per-name resolutions are folded into the decision JSONB (no separate `name_matches` table)
- [x] **LLM-09**: Reconciliation enforces a one-to-one roster mapping — a duplicate submitted name, two submitted names resolving to the same employee, or a name resolving to no roster employee (or to 2+ employees) gates the run to clarification (a name cannot silently collapse onto another employee). This is a pure run-level code check in `decide.py` (Phase 2.1)
