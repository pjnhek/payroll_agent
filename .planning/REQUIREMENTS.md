...
- [ ] **OPS2-01**: A failed run records a PII-safe error detail (sanitized exception message + context), not just the exception type — so production failures are diagnosable from the dashboard/DB without log access. (Audit HIGH-05, `record_run_error` + schema.)
