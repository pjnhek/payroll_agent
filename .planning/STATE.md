...
| 260726-sje | Landing page (/): restructure as a proof surface — lead with the gate-tripping `unknown_shorthand_metro` fixture behind a named `LANDING_GATE_FIXTURE_KEY` constant (pinned by test to the allowlist AND to the fixture's own request_clarification expectation, closing a silent-fallback trap where a rename would flip the demo into its opposite), render the fixture's actual email escaped via the server-side allowlist, card the roster + composer, cut eyebrows 3→1, remove the accent play-circle so exactly one accent CTA remains, and add a `<noscript>` submit fixing a pre-existing JS-off failure in the business picker — display only, 0 pipeline change; group 2 of 3 from the /impeccable critique | 2026-07-26 | 28cd06c | [260726-sje-landing-page-restructure-as-a-proof-surf](./quick/260726-sje-landing-page-restructure-as-a-proof-surf/) |
| 260726-tog | Design system: drop the render-blocking Google Fonts request for the platform-native UI stack, and replace the provisional indigo accent with Ledger Teal `#0F5F5C`. The accent VACATES indigo rather than pending being retuned — indigo becomes the waiting family's exclusive property via new `state-pending-*` tokens, so 4 hard-coded literals became named tokens and `--accent-soft` was deleted. `eval/chart.svg` regenerated onto the new accent via the `_write_svg_chart` route (NOT `--chart`, which rewrites `summary.json` with a fresh suite_run_id as a side effect); `summary.json` byte-identical, `--check` green. Contrast computed and made durable by `tests/test_design_tokens.py`, which parses `:root` and recomputes ratios (white-on-accent 7.47:1, accent-on-ground 7.03:1, both beating the indigo baselines). DESIGN.md + design.json + PRODUCT.md corrected where they had gone false. **Outstanding: visual reflow check at 640/820/960/1280 not performed — no browser available.** Group 3a of the /impeccable critique | 2026-07-26 | 48e0d46 | [260726-tog-drop-the-google-fonts-webfont-for-a-nati](./quick/260726-tog-drop-the-google-fonts-webfont-for-a-nati/) |
| 260726-ugm | Narrow-width adaptation + polish: breakpoint grown 2→10 rules (shell inset, card padding, form wrap, fixed-width selects, ops/eval grid strips, table min-width) with 4 wide tables in focusable `.table-scroll` regions; per-page `<title>`s, JS-free `aria-current` nav state, one-base button composition (`.btn` + non-cloning modifiers, money-gate class allowlisted to exactly one site), zero inline styles on index.html. **Key finding: 3 of the 4 "class with no CSS" items (`status-badge`/`failure-summary`/`failure-secondary`) were LIVE poller JS hooks, not dead markup — deleting them as DESIGN.md's own Don't invited would have broken status polling; renamed to `js-` prefix with the convention recorded as a named rule, and that Don't corrected.** Also: an inline `grid-template-columns` on ops.html outranked the media query and would have silently defeated the stacking fix. Post-redirect failure gap needed no feature (already covered by test_dashboard.py:480); added one retrigger-affordance assertion instead. **VISUALLY VERIFIED 2026-07-27** (was outstanding at execution: no browser). Chrome extension connected, app run locally with `WORKER_COUNT=0` (no queue drain, no outbound email, 0 POSTs, read-only). At a real 375px viewport: 0px horizontal overflow, 0 elements wider than the viewport, nav/wrapper padding collapsed 64px→16px, select `min-width` unpinned to 0. `/eval`'s 1024px table scrolls inside its `overflow-x:auto` `.table-scroll` region (`tabIndex=0`, keyboard-reachable) with the page itself not overflowing. Live computed contrast: white-on-accent 7.469:1, `.form-help`/`.column-label` 4.834:1, `.lede` 4.550:1 (the marginal one, confirmed), disclaimer 15.916:1. Per-page `<title>` and nav `aria-current` correct on all 4 pages. Group 3b of 3 | 2026-07-26 | 727d9c6 | [260726-ugm-narrow-width-adaptation-cold-start-reass](./quick/260726-ugm-narrow-width-adaptation-cold-start-reass/) |
| 260816-ee8 | Fix 6 findings from an adversarial pre-push review of `8f42d60..HEAD`. **(1) The terminal-vs-retryable classification reintroduced the dead end it was written to kill:** hiding "Authorize a new confirmation" for `authorization`/`validation`/`configuration` left "Mark delivered" (which CASes to RECONCILED) as the operator's ONLY action on a confirmation the provider had definitively refused — the sole exit required recording a falsehood. Reject now renders on that branch only; on the retryable branch delivery may genuinely have happened, so adding it there would be a new hazard, not a fix. **(2+3) The BUG-13 guard did not do what its comment claimed:** deleting the `[Your Name]` TOKEN left `Best regards,` dangling over nothing (the exact symptom BUG-13 was reported for), and the blanket bracket regex silently ate legitimate prose (`net pay for [pay period ending 2026-06-15]` → `net pay for  `). Now truncates the sign-off BLOCK, and a residual placeholder DISQUALIFIES the draft to the template floor rather than editing it. Separately the prompt claimed "the system appends its own closing line" while nothing appended anything — the drafted path ended on a dollar figure while the FALLBACK was the only path that closed properly; one `_CONFIRMATION_CLOSING` constant now serves both. **(4)** `reject()`'s docstring claimed an awaiting_reply run holds no in-flight send — the opposite of the code (`clarification.py` sets AWAITING_REPLY in the same transaction that enqueues SEND_OUTBOUND), replaced with the real downstream fences + a mutation-proofed test. **(5)** `render.yaml` DEMO_OUTBOUND_TO → `sync: false`. **Outstanding: `.env.example` blocked by a dotenv guard (manual paste, line in SUMMARY.md); and the address is NOT scrubbed — `app/routes/demo.py:72` still hard-codes it as `DEMO_OPERATOR_EMAIL`.** Invariants re-verified untouched: at-most-once send, `send_state='sent'` proof filter, `approve()`, `?notice=` allow-listing, record-only fencing. 1390→1400 tests | 2026-08-16 | 1132fdc | [260816-ee8-fix-six-adversarial-pre-push-review-find](./quick/260816-ee8-fix-six-adversarial-pre-push-review-find/) |
| 260816-ffv | Make the hardcoded demo operator email config-driven. `app/routes/demo.py:72` shipped `DEMO_OPERATOR_EMAIL = "pjnhek@gmail.com"` in a public repo, and it was not decorative: it is one of exactly four senders `find_business_by_sender` accepts, the other three being undeliverable `.example` seeds. **Hygiene, not a security fix** (the human approval gate carries the weight, and the address is in 19 public commits — history deliberately NOT rewritten). Now a `demo_operator_email` Setting resolved at CALL time via `resolve_operator_email()`, mirroring `app/email/routing.py::resolve_outbound_recipient` (same mailbox, outbound leg). **The constant could not simply be reassigned from settings:** `dashboard.py:12` did `from app.routes.demo import DEMO_OPERATOR_EMAIL`, binding a frozen copy at import, so no `setenv` + `cache_clear` could ever reach it — the accessor is what makes the value testable at all. **New refusal fence:** with no address configured, `/demo/bind` redirects to `?notice=demo_operator_unset` and never calls `bind_demo_business`, because a `demo_sender_bindings` row with `operator_email=''` would let `find_business_by_sender("")` resolve a business. Whitespace-only counts as unset. Guard is ordered SECOND, after the existing `demo_unknown_business` allowlist check, so that older test keeps its meaning. **DRY:** `scripts/demo_reset.py`'s parallel `DEMO_CONTACT_EMAIL` retired onto the same name (it was never in `render.yaml`, so local-only). Dead `demo_operator_email` template key deleted (zero consumers in `app/templates/`). RED-proof captured verbatim pre-guard. 1400→1403 tests | 2026-08-16 | bdb8bd9 | [260816-ffv-make-the-hardcoded-demo-operator-email-c](./quick/260816-ffv-make-the-hardcoded-demo-operator-email-c/) |
