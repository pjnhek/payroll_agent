"""Wave 0 RED test stubs — claim_status CAS helper (FOUND-04, D-12).
...
def test_claim_status_returns_true_when_row_returned():
    ...
    (the run was in the expected status — the claim succeeded, D-12/FOUND-04)."""
    conn = FakeConnection()
    ...
    assert result is True, (
        ...
        "(run was in expected status — CAS succeeded, FOUND-04/D-12)"
    )


def test_claim_status_returns_false_when_no_row():
    """claim_status returns False when the UPDATE returns no row
    (the run was NOT in the expected status — late/duplicate call, D-12)."""
    conn = FakeConnection()
    ...
    assert result is False, (
        ...
        "(run was not in expected status — duplicate dropped, D-12)"
    )
...
def test_claim_status_sql_contains_where_status_and_returning():
    ...
    predicate or the RETURNING clause is caught immediately (T-05-01)."""
    conn = FakeConnection()
    conn.script_fetchone(("some-uuid",))
    ...
    assert "AND status = %s" in sql_executed, (
        ...
        "that makes the operation atomic (T-05-01)"
    )
    assert "RETURNING" in sql_executed.upper() and "id" in sql_executed.lower(), (
        ...
        "distinguishes a successful claim from a no-op (T-05-01)"
    )
...
def test_claim_status_invariant_doc_updated():
    """Verify the 'two writers' invariant docstring will be planted in Wave 1.

    This test acts as a sentinel: once Wave 1 updates the repo module-level
    docstring or set_status docstring to say 'two writers', this test goes GREEN.
    Until then it stays RED, reminding the Wave 1 executor that the doc update
    is a required part of the D-12 implementation (not optional commentary).

    Per PATTERNS.md: 'two writers — set_status (unguarded) and claim_status
